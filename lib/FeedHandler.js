...
let FeedHandler = {
  ...
  clearSite: function(site) {
    delete storage.feedHandler[site];
  },
  ...
  rankNewsUpdate: function(data) {
    ...
    for (let i in this.rankers) {
      ...
      for (let j in docs) {
        let doc = docs[j];
        let docRank = ranker.rankSiteDocument(site, doc);
        if (docRank) {
          ...
          if (!siteEntry[doc.url]) {
            siteEntry[doc.url] = {
              ...
              summary: doc.content.substring(0, 256),
              ranks: {},
            };
          }
          siteEntry[doc.url].ranks[ranker.getName()] = docRank;
