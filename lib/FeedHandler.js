...
let FeedHandler = {
  ...
  getRankedDocsByRanker: function(site, rankerName) {
    let docs = [];
    Object.keys(this.feedHandler.sites[site]).forEach(url => {
      let doc = this.feedHandler.sites[site][url];
      if (doc.ranks[rankerName]) {
        docs.push(doc);
      }
    });
    return docs;
  ...
  getSiteNames: function() {
    return Object.keys(this.feedHandler.sites);
  },

  getRankerNames: function() {
    return this.rankers.map(ranker => {
      return ranker.getName();
    });
  },
