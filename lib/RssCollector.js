...
let RssCollector = {
  ...
  init: function() {
    ...
    Object.keys(this.collector.feeds).forEach(url => {
      ...
      dump(url + " has " + this.docCollectors[url].getSize() + " ===\n");
    });
        ...
        onComplete: function(response) {
          if (response.status == 200) {
            try {
              ...
              if (!feed.title) {
                dump(feed.url + " " + feed.title + "\n");
                //dump(JSON.stringify(feed,null,1) + "\n");
              }
            } catch (e) {
              dump(e + " ERROROR\n");
              deferred.reject(e);
  ...
  getFeedsArray: function() {
    return Object.keys(this.collector.feeds).map(url => {
      ...
      let docArray = Object.keys(docs).map(url => {return docs[url];}).sort((a,b) => {return b.score - a.score;});
      return {
         id: feed.id,
         url: url,
         tld: feed.tld,
