...
function DocCollector(storageKey, siteCollector) {
  if (storageKey) {
...
DocCollector.prototype = {
  ...
  rankDocument: function(doc) {
    ...
    doc.scoreKeys = [];
    let seen = {};
    let siteFeatures = this.collection.siteCollector.getFeatures();
    Object.keys(doc.features).forEach(token => {
       let fweight = this.getFeatureWeight(token) * (siteFeatures[token] && siteFeatures[token] > 0 ? siteFeatures[token]: 0);
  ...
  rankDocuments: function(recompute) {
    Object.keys(this.collection.docs).forEach(url => {
      ...
      if (this.collection.siteCollector.hasUrl(url)) {
  ...
  resetSiteFeatures: function(siteCollector) {
    this.collection.siteCollector = siteCollector;
  ...
  hasUrl: function(url) {
    return this.collection.docs[url] != null;
  },
