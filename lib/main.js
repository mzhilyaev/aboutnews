...
let NewsPage = {
  ...
  page: {
    ...
    contentScriptFile: [
      data.url("js/jquery-1.10.2.min.js"),
      data.url("js/angular.js"),
      data.url("js/newsAppControllers.js"),
      data.url("js/newsApp.js"),
    ],
    ...
    onAttach: function(worker) {
      ...
      worker.port.emit("import", data.load("docScoresView.html"));
      worker.port.emit("import", data.load("siteView.html"));
      worker.port.emit("bootstrap");
      ...
      worker.port.on("siteDocs", function(site) {
        worker.port.emit("siteDocs", {userDocs: HistoryCollector.getSiteHistoryDocs(site)});
      });

      // update site names and personilizers
      worker.port.on("updateNames", function() {
